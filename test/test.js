...
describe(
    '60 random tests', () => {
        ...
        // it(
        //     'Does not remove link when second link fails between 2 atoms', () => {
        //         f = () => m.brancher(2,1,3)
        //         .add([1,3,'P'],[1,1,'S'],[1,3,'H'])
        //         .closer().unlock()
        //         .bounder([1,2,2,3])
        //         .mutate([1,2,'O'],[3,3,'F'])
        //         .addChaining(1,1,'Mg','H')
        //         .add([1,2,'F'],[1,1,'Cl'])
        //         .addChaining(2,1,'S','O')
        //         .bounder([1,1,2,1])
        //         assert.throws(f, InvalidBond)
        ...
        //         assert.deepEqual(
        //             links,
        //             [ 'Atom(C.1: C2,Cl12,Mg9,S8)', 'Atom(C.2: C1,C5,S13,H)', 'Atom(O.3: C5,F11)', 'Atom(C.4: C5,P7,H,H)', 'Atom(C.5: C2,C4,O3,F6)', 'Atom(F.6: C5)', 'Atom(P.7: C4,H,H)', 'Atom(S.8: C1,H)', 'Atom(Mg.9: C1,H)', 'Atom(F.11: O3)', 'Atom(Cl.12: C1)', 'Atom(S.13: C2,O14)', 'Atom(O.14: S13,H)' ]
        //         )
        //     }
        // )
        ...
        it(
            'Does something unexpected', () => {
                f1 = () => m.brancher(8)
                .bounder([6,1,3,1],[7,1,8,1],[4,1,1,1],[6,1,1,1])
                .add([4,1,'O'],[1,1,'Cl'],[5,1,'S']).display().add([4,1,'C'])
                assert.throws(f1, InvalidBond)

                m.display().mutate([8,1,'C'],[2,1,'Mg'])

                const links = m.atoms.map(a => a.toString())

                assert.deepEqual(
                    links,
                    [ 'Atom(C.1: C4,C6,Cl10,Mg2)', 'Atom(Mg.2: C1,C3)', 'Atom(C.3: C4,C6,Mg2)', 'Atom(C.4: C1,C3,C5,O9)', 'Atom(C.5: C4,C6,S11)', 'Atom(C.6: C1,C3,C5,C7)', 'Atom(C.7: C6,C8,C8)', 'Atom(C.8: C7,C7)', 'Atom(O.9: C4)', 'Atom(Cl.10: C1)', 'Atom(S.11: C5)' ]
                )
